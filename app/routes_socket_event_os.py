...
from app.terminal_manager import terminal_manager
from app.docker_terminal_manager import docker_terminal_manager
...
#     pid = docker_terminal_manager.start_terminal(send_output)  # 터미널 세션 시작
#     print(f"클라이언트가 연결되었습니다. PID: {pid}로 터미널이 시작됨")

# @socketio.on('disconnect', namespace='/docker')
# def handle_disconnect():
#     """클라이언트 연결 해제를 처리합니다."""
#     docker_terminal_manager.stop_terminal()  # 터미널 세션 종료
#     print("클라이언트 연결이 해제되었습니다. 터미널이 종료됨")

# @socketio.on('execute_command', namespace='/docker')
# def handle_command(data):
#     """클라이언트로부터 명령어 실행 요청을 처리합니다."""
#     docker_terminal_manager.send_command(data['command'])  # 터미널에 명령어 전송

# @socketio.on('resize_screen', namespace='/docker')
# def handle_resize(data):
#     """클라이언트로부터 터미널 크기 조정 요청을 처리합니다."""
#     docker_terminal_manager.resize_terminal(data['rows'], data['cols'])  # 터미널 크기 조정